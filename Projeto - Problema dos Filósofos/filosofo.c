...
#define quant_filosofos 5 // Define constante de quantidade de filÃ³sofos
...
void pegaGarfo(int num_filosofo)
{
  ...
  printf("Filosofo de numero %d esta com fome \n",num_filosofo+1);// Exibe mensagem na tela de que o filÃ³sofo estÃ¡ com fome
  teste(num_filosofo); // Tenta fazer com que o filÃ³sofo pegue os garfos. 
  sem_post(&saleiro); // Destrava o semÃ¡foro saleiro
  sem_wait(&sem_filosofo[num_filosofo]); // Espera atÃ© o semÃ¡foro referente ao filÃ³sofo estar liberado
  sleep(1);
}
